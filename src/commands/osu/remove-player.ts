...
export default new Command("remove-player", "Deletes player", ApplicationCommandOptionType.Subcommand, [
    new Option("del", "Czy usunąć wszystkich graczy?", true, ApplicationCommandOptionType.Boolean),
    new Option("del_num", "Którego gracza usunąć?", false, ApplicationCommandOptionType.String)
    ...
    if(delete_bool == true){
        ...
        const idQuery = sql`SELECT id FROM players ORDER BY id DESC LIMIT 1`;
        const idQueryResult = db.get(idQuery) as Array<number>;
        const idQueryID = idQueryResult ? idQueryResult[0] : 0;

        const checkIfExists = sql`SELECT * FROM players WHERE id == ${delete_num}`;
